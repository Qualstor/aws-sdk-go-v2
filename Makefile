...
copy-attributevalue-feature:
	...
	ls *.go | grep -v "convert.go" | grep -v "doc.go" | \
		xargs -I % sed -i.bk -E 's:github.com/Qualstor/aws-sdk-go-v2/(service|feature)/dynamodb:github.com/Qualstor/aws-sdk-go-v2/\1/dynamodbstreams:g' % &&  \
	ls *.go | grep -v "convert.go" | grep -v "doc.go" | \
...
list-deps-%:
	...
	@cd ./internal/repotools/cmd/eachmodule \
		...
		"go list -m all | grep -v 'github.com/Qualstor/aws-sdk-go-v2'" | sort -u
