...
type UpdateProvisionedProductPropertiesInput struct {
	...
	// account. Both end user owners and administrators can see ownership history of
	// the provisioned product using the ListRecordHistory API. The new owner can
	// describe all past records for the provisioned product using the DescribeRecord
	// API. The previous owner can no longer use DescribeRecord, but can still see the
	// product's history from when he was an owner using ListRecordHistory. If a
	// provisioned product ownership is assigned to an end user, they can see and
